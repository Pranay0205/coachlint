...
async function process_hover_function_help(request, apiKey, model_name) {
  ...
  const ai_explanation = await generate_explanation(prompt, apiKey, model_name);

  return { message: ai_explanation };
}

async function process_hover_error_message(request, apiKey, model_name) {
...
async function process_code_review_message(request, apiKey, model_name) {
...
${request.code}`;
  ...
  const ai_response = await generate_explanation(prompt, apiKey, model_name);
